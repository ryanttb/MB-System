...
int mbr_em300mba_wr_bath(int verbose, FILE *mbfp, int swap, struct mbsys_simrad2_struct *store, int head, int *error) {
	...
	if (status == MB_SUCCESS) {
		...
		/* compute checksum */
		uchar_ptr = (mb_u_char *)line;
		for (int j = 0; j < EM2_BATH_MBA_HEADER_SIZE; j++)
			checksum += uchar_ptr[j];
	...
	if (status == MB_SUCCESS)
		for (int i = 0; i < ping->png_nbeams; i++) {
			...
			/* compute checksum */
			uchar_ptr = (mb_u_char *)line;
			for (int j = 0; j < EM2_BATH_MBA_BEAM_SIZE; j++)
				checksum += uchar_ptr[j];
...
int mbr_em300mba_wr_ss(int verbose, FILE *mbfp, int swap, struct mbsys_simrad2_struct *store, int head, int *error) {
	...
	if (status == MB_SUCCESS) {
		...
		/* compute checksum */
		uchar_ptr = (mb_u_char *)line;
		for (int j = 0; j < EM2_SS_MBA_HEADER_SIZE; j++)
			checksum += uchar_ptr[j];
	...
	if (status == MB_SUCCESS)
		for (int i = 0; i < ping->png_nbeams_ss; i++) {
			...
			/* compute checksum */
			uchar_ptr = (mb_u_char *)line;
			for (int j = 0; j < EM2_SS_MBA_BEAM_SIZE; j++)
				checksum += uchar_ptr[j];
